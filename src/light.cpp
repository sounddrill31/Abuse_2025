...
/**
 * Applies lighting remap table to a line of pixels
 * @param screen_addr Pointer to screen buffer to modify
 * @param remap Lighting remap table
 * @param w Width in pixels to process
 */
inline void MAP_PUT(uint8_t *screen_addr, const uint8_t *remap, int w)
{
  // For performance, we keep this simple and direct
  // Both pointers must be valid and w must be > 0
  while (w--)
  {
    *screen_addr = remap[*screen_addr];
    ++screen_addr;
  }
}
...
/**
 * Calculates lighting value for a specific screen location
 * Sums up contributions from all lights affecting this patch
 *
 * @param lp Light patch containing affecting lights
 * @param sx Screen X coordinate
 * @param sy Screen Y coordinate
 * @return Final light intensity value (0-63)
 */
inline int calc_light_value(const light_patch *lp, int32_t sx, int32_t sy)
{
  int light_value = min_light_level;
  int radius_squared;

  // Process each light source affecting this patch
  for (int i = 0; i < lp->total; ++i)
  {
    const light_source *current_light = lp->lights[i];

    // Type 9 is special case - solid rectangle light
    if (current_light->type == 9)
    {
      return current_light->inner_radius;
    }

    // Calculate distance from light to point
    int32_t dx = std::abs(current_light->x - sx) << current_light->xshift;
    int32_t dy = std::abs(current_light->y - sy) << current_light->yshift;

    // Approximate circular distance using quick calculation
    radius_squared = (dx < dy) ? dx + dy - (dx >> 1)  // When dy is larger
                               : dx + dy - (dy >> 1); // When dx is larger

    // Add light contribution if within range
    if (radius_squared < current_light->outer_radius)
    {
      // Calculate light intensity based on distance
      int intensity = ((current_light->outer_radius - radius_squared) * current_light->mul_div) >> 16;
