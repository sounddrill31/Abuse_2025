...
public:
  ...
  int operator==(const net_address &who) const { return equal(&who); }
...
public:
  ...
  int debug_level(const debug_type min_level) const { return min_level <= debug_setting; }
  void set_debug_printing(const debug_type level) { debug_setting = level; }
  ...
  virtual net_socket *start_notify(int port, void *data, int len) { return nullptr; }
  ...
  // Find methods
  virtual net_address *find_address(int port, char *name) { return nullptr; } // name should be a 256 byte buffer
  ...
  virtual ~net_protocol() { net_protocol::cleanup(); }
};
